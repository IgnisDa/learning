...
function ShowPage() {
  ...
  const { data: details, isLoading } = useQuery({
    queryKey: ["show-details", id, token],
    queryFn: async () => {
      return await authApiCall<any>(`/api/queries/shows/${id}`, token);
    },
  });
  ...
  const {
    ...
    mutationFn: async () => {
      return await authApiCall(`/api/tmdb/shows/${id}/refresh`, token, {
        method: "POST",
      });
    },
    onSuccess: () => {
      // Refetch show details after refresh
      queryClient.invalidateQueries({ queryKey: ["show-details", id, token] });
    },
  });
